...
class ResearchVessel:
    ...
    def fetch_file_objects_django(self): # джанго очень чувствителен к формату входящей информации, нужно все подогнать под его требования
        ...
        for info in self.fetch_file_objects():
            ...
            yield info_copy
        """    to_convert=["last_modified", "created_time", "Office_last_mod_time"]
            for field in to_convert:
                if info[field]:
                    info_copy[field]=format_date(info[field])      """            
